...
const ProfileUpdate = () => {
  ...
  const handlePhotoUpload = async (e, type) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/auth/upload-${type}-pic`, {
        method: 'POST',
        credentials: 'include',
        body: formData,
      });
      ...
      if (response.ok) {
        setUser(prev => ({
          ...prev,
          [type === 'profile' ? 'profilePic' : 'coverPic']: data[type === 'profile' ? 'profilePic' : 'coverPic']
        }));
          ...
          <div className="bg-gray-800 p-6 rounded-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                />
                <div className="text-sm text-gray-400 mt-1">{formData.bio.length}/500</div>
              </div>
